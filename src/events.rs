...
// pub async fn handle_lease_requests(key:Key,entry:Entry,bp_tree:&mut BPTree,
//     block_migration:&HashSet<BlockId>,commands:&mut HashMap<BlockId,Vec<GeneralRequest>>,client:&mut Client){
...
pub async fn handle_migrate(mut block:Block,bp_tree:&mut BPTree,channel: ResponseChannel<GenericResponse>,client:&mut Client){
    let id = block.return_id();
    bp_tree.add_block(id, block);
    client.respond(GeneralResponse::MigrateResponse,channel).await;
